...
This directory contains the comprehensive Model Context Protocol (MCP) integration for Argus. MCP enables Argus to seamlessly connect with and utilize external tools and data sources through a standardized protocol.

## 🔧 What is MCP?

Model Context Protocol (MCP) is an open standard that enables secure connections between AI applications and external data sources and tools. With MCP, Argus can:
...
- Converts Argus configs to MCP format
...
```python
from argus.mcp import MCPToolSelector
...
```python
from argus.mcp import MCPResearchSkill
...
```python
import logging
logging.getLogger('argus.mcp').setLevel(logging.DEBUG)
```
...
- **Argus Docs**: [Documentation](https://docs.gptr.dev/)
...
*This MCP integration brings powerful extensibility to Argus, enabling connections to virtually any data source or tool through the standardized MCP protocol.* 🙂 